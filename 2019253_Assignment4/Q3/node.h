...
public:
  ...
  void sendMsg()
  {
    for (int i = 0; i < interfaces.size(); ++i)
    {
      ...
      for (int j = 0; j < mytbl.tbl.size(); ++j)
      {
        ...
        new_entry.dstip = mytbl.tbl[j].dstip;
        new_entry.ip_interface = mytbl.tbl[j].ip_interface;
        new_entry.nexthop = mytbl.tbl[j].nexthop;
        msg.recvip.compare(new_entry.nexthop) == 0 ? new_entry.cost = 16 : new_entry.cost = mytbl.tbl[j].cost;
        new_table.tbl.push_back(new_entry);
      }
      msg.mytbl = &new_table;
  ...
  void updateTblEntry(string ip, string connip)
  {
    //a routing table for each node. change routing table entry of nodes B and C
    for (int i = 0; i < mytbl.tbl.size(); ++i)
    {
      if (mytbl.tbl[i].ip_interface.compare(ip) == 0 && mytbl.tbl[i].dstip.compare(connip) == 0)
      {
        mytbl.tbl[i].cost = 16;
      }
    }
    ...
    for (int i = 0; i < interfaces.size(); ++i)
    {
      if ((interfaces[i].first.getip().compare(ip) == 0 && interfaces[i].first.getConnectedIp().compare(connip) == 0) || (interfaces[i].first.getip().compare(connip) == 0 && interfaces[i].first.getConnectedIp().compare(ip) == 0))
      {
